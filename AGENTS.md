...
npm run preview          # Alias of dev — build, then serve dist/ at localhost:3456
npm run deploy           # npx wrangler deploy — pushes dist/ to Cloudflare Pages per wrangler.toml
```

No linting tools or tests are configured. The token build's contrast gate is the closest thing to a test suite — treat a failing gate as a build break. `dist/` is gitignored and fully generated by `npm run build` — never commit it or hand-edit its contents.
...
**Distribution:** npm publishing is not set up for now — the jsDelivr CDN link (see `README.md` Quick Start) is the only supported way consumers pull the tokens. A token rename or removal is still a breaking change regardless of channel — bump major and add a migration note to `CHANGELOG.md`.
